...
class ConvLayer(nn.Module):
    def __init__(self, in_c, out_c, kernel_size):
        ...
        self.conv = nn.Conv2d(
            in_c, out_c, kernel_size=kernel_size, stride=1, padding=pad)
...
class HRNet(nn.Module):
    def __init__(self):
        ...
        self.downsample2_1 = nn.Conv2d(
            16, 32, kernel_size=3, stride=2, padding=1)

        self.layer3_1 = Bottleneck(32, 32)
        self.layer3_2 = Bottleneck(32, 32)
        self.downsample3_1 = nn.Conv2d(
            32, 32, kernel_size=3, stride=2, padding=1)
        self.downsample3_2 = nn.Conv2d(
            32, 32, kernel_size=3, stride=4, padding=1)
        self.downsample3_3 = nn.Conv2d(
            32, 32, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        map3_2 = torch.cat(
            (self.downsample3_1(map2_1), self.layer3_2(map2_2)), 1)
        map3_3 = torch.cat((self.downsample3_2(map2_1),
                           self.downsample3_3(map2_2)), 1)

        map4_1 = torch.cat((self.layer4_1(map3_1), upsample(2)
                           (map3_2), upsample(4)(map3_3)), 1)
